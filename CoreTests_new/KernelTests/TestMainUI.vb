...
Public Class TestMainUI
    ...
    Public Sub TestMRUElements()
        ...
        Dim nextNr As Integer = CInt(1 + Rnd(Now.Millisecond) * m_Application.Adressen.Count - 1) Mod m_Application.Adressen.Count
        Dim testobj As StaticItem = m_Application.Adressen(nextNr)
        ...
        nextNr = CInt(1 + Rnd(Now.Millisecond) * m_Application.Adressen.Count - 1) Mod m_Application.Adressen.Count
        testobj = m_Application.Adressen(nextNr)
        mru.AddMRUElement(testobj)

        nextNr = CInt(1 + Rnd(Now.Millisecond) * m_Application.ArticleList.Count - 1) Mod m_Application.ArticleList.Count
        testobj = m_Application.ArticleList(nextNr)
        mru.AddMRUElement(testobj)

        nextNr = CInt(1 + Rnd(Now.Millisecond) * m_Application.JournalDocuments.Count - 1) Mod m_Application.JournalDocuments.Count
        testobj = m_Application.JournalDocuments(nextNr)
        mru.AddMRUElement(testobj)

        System.Threading.Thread.Sleep(2000) ' warten, damit eine Zeitdifferenz entsteht

        nextNr = CInt(1 + Rnd(Now.Millisecond) * m_Application.CashJournal.Count - 1) Mod m_Application.CashJournal.Count
        testobj = m_Application.CashJournal(nextNr)
        mru.AddMRUElement(testobj)
